...
def mock_erroneous_query(response_body, status):
    ...
    responses.add(responses.GET, query_url, body=response_body,
...
def mock_erroneous_upload(response_body, status):
    ...
    responses.add(responses.POST, contract_url, body=response_body,
                  status=status, content_type='application/json')
    data_client = DataClient(contract_id, 'contract_token', api_base=api_base)
    data_client.upload(filename='tests/data/mock_data.csv')
...
class ErrorTestCase(unittest.TestCase):
    ...
    def test_auth_error(self):
        with self.assertRaises(AuthenticationError):
            mock_erroneous_retrieve(
                '{"error":"not_found","logged_in":false}', 404)
        with self.assertRaises(AuthenticationError):
            mock_erroneous_retrieve(
                '{"error":"server_error","logged_in":false}', 500)

    @responses.activate
    def test_notfound_error(self):
        with self.assertRaises(NotFoundError):
            mock_erroneous_retrieve(
                '{"error":"not_found","logged_in":true}', 404)
