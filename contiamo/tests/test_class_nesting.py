...
class ClassNestingTestCase(unittest.TestCase):
  ...
  def test_nested_urls(self):
    self.assertEqual(self.project.instance_url(), '%s/%s' % (self.api_base, self.project_id))
    self.assertEqual(self.dashboard.instance_url(), '%s/%s/%s' % (self.project.instance_url(), 'dashboards', self.dashboard_id))
