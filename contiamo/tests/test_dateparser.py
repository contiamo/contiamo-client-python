...
class DateParseTestCase(unittest.TestCase):
  ...
  def test_parsing(self):
    for format in self.formats:
      ...
      self.assertEqual(date.strftime('%Y-%m-%dT%H:%M:%S'), format[2], '{0} parsing failed'.format(format[1]))
  ...
  def test_formatting(self):
    for format in self.formats:
      ...
      parser.identifyPeriodUnit(format[0])
      date = parser.parse(format[0])
      self.assertEqual(parser.format(date), format[0], '{0} formatting failed'.format(format[1]))
